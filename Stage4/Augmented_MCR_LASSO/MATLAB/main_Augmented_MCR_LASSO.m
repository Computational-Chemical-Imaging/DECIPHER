...
% Number of datapoints for augmentation, default is 1*NxNy
augnum  = 1*Nx*Ny;         
...
[C, ref_new] = ALS_lasso_aug(y_sub, ref, l, augnum, iter);

% Calculate the residual of unmixing
C_2D = reshape(C, [], k);
y_recon_2D = C_2D * ref_new.';
...
figure;
for ii = 1:k
    plot(Wn, ref_new(:,ii) + (ii-1), 'LineWidth', 1); hold on
end
hold off
set(gca,'XMinorTick','on','YMinorTick','on');
xlabel('Raman shift (cm^{-1})'); ylabel('Int (a.u.)')
...
for ii = 1:k
    ...
    outSpecFile = fullfile(opt_filepath, [cname, '_out.csv']);
    outSpecData = [(1:Nz)',ref_new(:,ii)];
    writematrix(outSpecData, outSpecFile); 
end
